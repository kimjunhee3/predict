...
from bs4 import BeautifulSoup  # NAVER 스냅샷 사용할 때만 필요
import requests
...
def index():
    ...
    # 1) 원격 JSON → 로컬 캐시(/data) 로드
    cache_json = fetch_remote_into_cache(force=False)

    # 2) 오늘자(없으면 최신) predlist를 얻어와서 선택팀 경기 찾기
    ...
    # 4) 퍼센트 포맷팅 (template에서 쓰는 필드 만들어 주기)
